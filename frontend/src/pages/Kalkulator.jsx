...
function Kalkulator() {
  ...
  const sourceLabel = latestGold?.source === 'logammulia' ? 'Logam Mulia' : (latestGold?.source ?? 'Logam Mulia');

  const totalEmas = (parseFloat(gram) || 0) * pricePerGram;
  const totalGram = pricePerGram > 0 ? ((parseFloat(uang) || 0) / pricePerGram).toFixed(2) : '0.00';
    ...
    <>
      ...
      <div className="page-hero">
        <div className="wrap2">
          ...
          <p>Hitung nilai emas Anda dalam rupiah secara instan berdasarkan harga pasar terkini</p>
        </div>
      </div>

      <div className="main">
        <div className="mode-toggle">
          <button type="button" className={'mode-btn' + (mode === 'emas' ? ' active' : '')} onClick={() => setMode('emas')}>
            Emas → Uang
          </button>
          <button type="button" className={'mode-btn' + (mode === 'uang' ? ' active' : '')} onClick={() => setMode('uang')}>
            Uang → Emas
          </button>
        </div>
        ...
        <div className="calc-grid">
              ...
              <div className="calc-card">
                ...
                <div className="field">
                  ...
                  <div className="input-wrap">
                    <input type="number" placeholder="0" value={gram} onChange={(e) => setGram(e.target.value)} min="0" step="any" />
                ...
                <div className="result-box">
                  ...
                  <div className="r-val">{priceLoading ? 'Memuat…' : formatRupiah(totalEmas)}</div>
                  <div className="r-sub">
            ...
            {mode === 'uang' && (
              <div className="calc-card">
                ...
                <div className="field">
                  ...
                  <div className="input-wrap">
                    <input type="number" placeholder="0" value={uang} onChange={(e) => setUang(e.target.value)} min="0" step="1" />
                ...
                <div className="info-rows">
                  ...
                  <div className="info-row">
                    ...
                    <strong>{Math.round(parseFloat(totalGram) * 1000).toLocaleString('id-ID')} mg</strong>
                  </div>
                  <div className="info-row">
                    <span>Nilai buyback est.</span>
                    <strong>Rp {Math.round((parseFloat(uang) || 0) * 0.93).toLocaleString('id-ID')}</strong>
                  </div>
                  <div className="info-row">
                    <span>Harga/gram (IDR)</span>
                    <strong>{priceLoading ? '…' : formatRupiah(pricePerGram)}</strong>
                ...
                <p className="note">* Perhitungan menggunakan harga referensi terkini. Harga buyback biasanya 7–10% lebih rendah.</p>
              </div>
            )}
          </div>
          ...
          <div>
            <div className="ref-card">
              ...
              <p className="sub">{updatedLabel ? `Contoh harga beli resmi Logam Mulia · ${updatedLabel}` : 'Harga beli resmi Logam Mulia (contoh referensi)'}</p>
                <div className="ref-table-wrap" role="region" aria-label="Tabel referensi harga emas (geser kanan/kiri)">
              ...
              <p className="note calc-ref-note">Kalkulator memakai harga referensi API ({formatRupiah(pricePerGram)}/gram). Sedangkan tabel di atas ilustrasi spread resmi Antam.</p>
            </div>
            ...
            <div className="kurs-card">
              ...
              <p className="kurs-updated">{updatedLabel ?? 'Data kurs global: contoh referensi'}</p>
            </div>
          </div>
        </div>
      </div>
