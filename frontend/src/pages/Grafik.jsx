...
import { getGoldHistory, getLatestGoldPrice, getGoldStatsSummary, GRAFIK_TAB_LIMITS } from '../api/goldenticsApi.js';
import { formatRupiah, formatPercent } from '../utils/format.js';
import { buildGoldLineChart, computePeriodSummary, formatGoldDateId } from '../utils/goldChart.js';
import './Grafik.css';
...
function Grafik() {
  ...
  const latestDailyChange = latest?.dailyReturn != null ? formatPercent(latest.dailyReturn * 100) : summary?.dailyReturnPct != null ? formatPercent(summary.dailyReturnPct) : null;
  const latestChangeUp = (latest?.dailyReturn ?? summary?.dailyReturnPct ?? 0) >= 0;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const [hist, latestPrice, stats] = await Promise.all([getGoldHistory({ period: 'daily', limit: limitByTab }), getLatestGoldPrice(), getGoldStatsSummary(7)]);
  ...
  return (
    <>
      ...
      <StatsBar latest={latest} summary={summary} loading={loading} error={error} />
          ...
          <div className="chart-card">
            <div className="chart-header">
              <div>
                ...
                <div className="chart-sub">Harga penutupan (close) per hari · {activeTab}</div>
              </div>
            ...
            <div className="chart-wrap">
              {loading ? (
                ...
                <>
                  <div className="chart-tooltip">{formatRupiah(chart.lastPoint.price)}</div>
                  <svg viewBox={`0 0 ${chart.width} ${chart.height}`} preserveAspectRatio="none">
                    ...
                    {chart.yTicks.map((tick) => (
                      <g key={tick.label}>
                        <line x1={chart.padLeft} y1={tick.y} x2={chart.width - chart.padRight} y2={tick.y} stroke="#f0f0f0" strokeWidth="1" />
                        <text x="0" y={tick.y + 3} fontSize="9" fill="#bbb" fontFamily="Poppins,sans-serif">
                          {tick.label}
                        </text>
                      </g>
                    ))}
                    <path d={chart.areaD} fill="url(#g1)" />
                    <path d={chart.lineD} fill="none" stroke="#C9910A" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                    <circle cx={chart.lastPoint.x} cy={chart.lastPoint.y} r="5" fill="#C9910A" stroke="#fff" strokeWidth="2" />
                    {chart.xLabels.map((lbl) => (
                      <text key={lbl.label} x={lbl.x} y={chart.height - 4} fontSize="9" fill="#bbb" fontFamily="Poppins,sans-serif" textAnchor="middle">
                        {lbl.label}
                      </text>
                    ))}
                  </svg>
          ...
          <div className="sidebar">
            <div className="side-card">
              ...
              <div className="price-now">
                <div className="price-now-lbl">Logam Mulia</div>
                <div className="price-now-val">{latest ? formatRupiah(pricePerGram) : '—'}</div>
                {latestDailyChange ? (
              ...
              <div className="price-rows">
                {gramMultipliers.map((g) => (
                  <div key={g} className="price-row">
                    ...
                    <span className="price-row-val">{latest ? formatRupiah(pricePerGram * g) : '—'}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                {!periodSummary ? (
                  ...
                  <>
                    ...
                    <div className="sum-row">
                      ...
                      <span className={`sum-val ${periodSummary.change >= 0 ? 'up' : 'down'}`}>
                        {periodSummary.change >= 0 ? '+' : ''}
                        {formatRupiah(periodSummary.change)} ({formatPercent(periodSummary.changePct)})
                      </span>
                    </div>
                  </>
                )}
      ...
      <div className="table-section">
        <div className="table-card">
          ...
          <div className="tbl-scroll" role="region" aria-label="Tabel histori harga emas">
            <table className="tbl">
              <thead>
                <tr>
                  <th>Tanggal</th>
                  ...
                  [...history].reverse().map((row, idx) => {
                    ...
                    return (
                      <tr key={`${row.date}-${idx}`}>
                        ...
                        <td>{row.volume != null ? row.volume.toLocaleString('id-ID') : '—'}</td>
